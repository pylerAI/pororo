...
def applyLM(parentBeam, childBeam, classes, lm_model, lm_factor: float = 0.01):
    ...
    if lm_model is not None and not childBeam.lmApplied:
        ...
        history = " ".join(classes[each].replace(" ", "▁") for each in history if each != 0)
...
def get_image_list(horizontal_list: list, free_list: list, img: np.ndarray, model_height: int = 64):
    image_list = []
